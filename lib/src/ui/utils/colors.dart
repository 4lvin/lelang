import 'package:flutter/material.dart';

class Colorses{
  final Color hijauDasar = Color(0xff009B87);
  final Color hijauDurasi = Color.fromRGBO(0,191,165, 1);
  final Color hijauHarga = Color.fromRGBO(24,152,131, 1);
  final Color tombolaktif = Color.fromRGBO(18, 112, 97, 1);
  final Color merahLight = Colors.red[300];
  final List<Color> gradient = [Color(0xff009B87), Color(0xff9DD3CB)];
}
final colorses = Colorses();